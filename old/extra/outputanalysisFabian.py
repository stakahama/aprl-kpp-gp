...
whole_datax = pd.DataFrame.from_csv(keyword+'formatted.txt', header=0, index_col=0)
...
if aggregation_method == 'mean':
    ...
    dataxmean = pd.DataFrame(temporary_df.groupby(qcut(temporary_df.index,plot_n_values,labels=False)).mean()) # take the grouped mean, group by intervals created with qcut
