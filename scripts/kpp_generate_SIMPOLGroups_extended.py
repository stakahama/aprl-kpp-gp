...
## June 2014
##
## extended by F.Bernhard 02.01.15 in order to generate molecular masses
##
################################################################################
...
class kppParameters:
    ...
    def __init__(self,root,path='.'):
        ## root is the value of ROOT used as example in KPP
        ##   manuscript by Sandu
        ## path is for both ROOT_Parameters.f90 file (input)
        ##   and ROOT_SIMPOLGroups.f90
        self.root = root    #FB (29.12.14)
        self.path = path
    ...
    def read_smiles_table(self,smilesfile,attr='smiles',columns=['compound','SMILES','InChI','molwt']):
        ...
        with open(smilesfile) as f:
            ...
            for line in f:
                if len(line) > 2:
                    fields.append(tuple(map(str.strip,line.strip().split('\t'))))
