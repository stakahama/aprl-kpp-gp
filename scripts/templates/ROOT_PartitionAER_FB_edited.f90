...
! modules
    ...
    use {ROOT}_GlobalAER, only: CAER, VPAER, NorganicSPEC, organic_selection_indices
    use {ROOT}_monitor, only: SPC_NAMES ! FB: to plot the spc_names

    real(kind=dp) ::  T,TOUT
! local variables
    integer :: timesteps, current_step, j, i
    real (kind=dp)  :: MCM_timestep_size
    real (kind=dp), dimension(NSPEC, 2) :: intermediate_CGAS, intermediate_CAER       ! intermediate CAER and CGAS stock the integrated values in case multiple partitioning time steps are done within one MCM timestep
    ...
    integer :: displayed_not_enough_CAER(NSPEC)   !FB: flag to count how many times the error message on not enough CAER has already been shown f$
    integer :: displayed_not_enough_CGAS(NSPEC)   !FB: flag to count how many times the error message on not enough CAER has already been shown f$
    
    integer :: organic_selection_binary(NSPEC)
! body

        organic_selection_binary=0
        do i = 1,NorganicSPEC
          ! define it as organic
          organic_selection_binary(organic_selection_indices(i)) = 1
       enddo

    timesteps = 1
    MCM_timestep_size = TOUT-T
    ...
    ! INTEGRATE
      ...
      do current_step = 1,timesteps
          ...
          call integrate(CGAS_old, CAER_old, CGAS_new, CAER_new, PARTITION_timestep_size, current_step, &
               displayed_not_enough_CGAS, displayed_not_enough_CAER, organic_selection_binary)
...
!    end do
  ...
  subroutine integrate(CGAS_old, CAER_old, CGAS_new, CAER_new, dt, current_step, &
       displayed_not_enough_CGAS, displayed_not_enough_CAER, organic_selection_binary)
    ...
    use {ROOT}_GlobalAER, only: VPAER, NorganicSPEC, molecular_masses, &
    ...
    integer, intent(out) :: displayed_not_enough_CAER(NSPEC)   !FB: flag to count how many times the error message on not enough CAER has already been shown f$
    integer, intent(out) :: displayed_not_enough_CGAS(NSPEC)   !FB: flag to count how many times the error message on not enough CAER has already been shown f$
    integer, intent(in) :: organic_selection_binary(NSPEC)
...
! body
    ...
    ! B) INTEGRATION
     change = 0
      ...
      factor = dt*2*pi*nr_aerosol_particles*d_ve_aerosols*f_a_Kn*Diff_coeff;

      select case (integrator)
          case ('backward')
              write(*,*) 'using backward'
              ! use 'semi-implicit' Backward Euler (deviates from fully implicit by evaluating a = CAER_new/sum(CAER_old))
              ! coupled equation to solve
                  ! CGAS_new = (CGAS_old - factor*VPAER*CAER_new/sum(CAER_old))/(1-factor)
                  ! CAER_new = (CAER_old + factor*C1)/(1+factor*VPAER/sum(CAER_old))
            
              CGAS_new = (CGAS_old/(1-factor) - (CAER_old*factor*VPAER)/((1-factor)*(sum_organic_CAER_old*factor*VPAER))) &
                          * (1/(1+(factor)/((1-factor)*(sum(CAER_old)*factor*VPAER))))
              change = CGAS_old-CGAS_new;
          
          case ('forward')
              write(*,*) 'using forward'
              change = factor * (CGAS_old - CAER_old/sum_organic_CAER_old*VPAER);
    ...
    ! C) FINAL CHECK for available amount of each compound and correct (only for forward and backward)
      select case (integrator) ! select was used to easily switch off the check for dlsode. At first it was thought unnecessary, but it seems to be better if we still do it.
        case ('backward','forward','dlsode')
