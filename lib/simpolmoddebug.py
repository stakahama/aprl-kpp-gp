...
"""
        ...
        self.smartspatt = OrderedDict([
            ...
            ('_carbonylperoxynitrate','C(=O)OO[N+](=O)[O-]'),
    ...
    def get_groups(self,smilesstr=None):
        ...
        ## calculate SIMPOL groups from auxiliary variables        
       ...
       #nonaromatic rings (AC)
        ...
        print smilesstr, abundances['_aromatic ring'], aux['_edges'], aux['_atoms'], abundances['_non-aromatic ring']

        
        abundances['_ether'] = max(aux['_ether_all'] \
                             - abundances['_ester'] \
                             - abundances['_ether (alicyclic)'] \
                             - abundances['_ether, aromatic'],0)
    ...
    def get_FTIRgroups(self,smilesstr=None):
        ## takes a single SMILES string and returns an array
        ##   of group abundances for SIMPOL
        ##
        ## if smilesstr is none, return array of zeroes
        if not smilesstr:
        ...
        aux['H'] = aux['h1']+2*aux['h2']+3*aux['h3']+4*aux['h4'] 
        
        aux['alkane CH'] = aux['tertiary C'] + aux['CH2']*2 + aux['CH3']*3
        
        aux['alkene CH'] = aux['alkene CH1'] + aux['alkene CH2']
                
        aux['ether'] = max(aux['ether']-aux['ester'],0)

        header = ['C','O','H','secondary amine','tertiary amine','CH2','CH3',
                  'tertiary C','primary amine','alkane CH','alkene CH',
                  'aromatic CH','carbonyl','alcohol','carboxylic acid',
                  'ester','ether']
    ...
    def calc_properties(self,smiles,temp):
        ...
        props = []
        for name in smiles:
            abundances = self.get_groups(name)
            if any(abundances[1:] > 0):
                props.append(self.p0_atm(abundances,temp))
                props.append(self.deltaHvap_kJpermol(abundances,temp))
        return props
...
if __name__=='__main__':
    ...
    simp = Simpolclass()
    print simp.get_FTIRgroups('CC(=CC=O)C(=O)OO')
